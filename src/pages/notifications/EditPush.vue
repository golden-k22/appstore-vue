<template>
  <div class="edit-push-box">
    <div class="edit-push-body">
      <!-- ================= header ==================-->
      <el-row>
        <div class="edit-push-header">
          <label class="edit-push-point">&#9679;</label>
          <label class="edit-push-title">修改推送</label>
        </div>
      </el-row>
      <!-- =========== end header =================== -->
      <!-- ==============  body ===================== -->
      <el-row>
        <div style="padding:50px" class="block-inside bg-white">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="edit-push-form">
            <div class="row">
              <div class="col-sm-4">
            ...
            <!--  ============== nduction editor ============== -->
            <div class="edit-push-height-200" style="width:100% height:100%">
            ...
            <div class="collect-text-align-center margin-top-40">
              <el-button
                type="primary"
                @click="submit"
                class="edit-push-btn-primary pull-right"
                round
              >发布</el-button>
              <el-button
                type="danger"
                @click="refresh"
                class="edit-push-btn-danger pull-right"
                round
              >清空</el-button>
            </div>
          </el-form>
...
export default {
  ...
  async created() {
    this.id = this.$route.params["pushId"];
  },
  ...
  methods: {
    ...
    async submitPush() {
      ...
      if (this.id !== undefined) {
        this.$store.commit("Notification/setId", this.id);
...
.edit-push-height-200 .ck.ck-editor__editable_inline {
  height: 300px;
}

.edit-push-btn-danger {
  width: 130px;
  margin-right: 20px;
}
.edit-push-form .el-form-item__error {
  color: #f56c6c;
  font-size: 12px;
  line-height: 1;
  padding-top: 4px;
