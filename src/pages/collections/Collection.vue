    ...
    <div class="collection-body">
      ...
      <el-row>
        <div class="collection-header">
          ...
          <label class="collection-title">发布专题</label>
        </div>
      </el-row>
      <!-- =========== end header =================== -->
      <!-- ==============  body ===================== -->
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
            ...
            <!-- <el-row>
              <el-form-item label="Category" prop="CATEGORY">
                <el-select v-model="categ" filterable placeholder="Choose Category">
                  <el-option
                    v-for="item in itemCategories"
                    :key="item.categoryId"
                    :label="item.categName"
                    :value="item.categId"
                  ></el-option>
                </el-select>
              </el-form-item>
              ...
              <el-row>
                ...
                <el-col :span="3" class="m-3">
                  <el-button
                    ...
                    v-if="files[0] !== undefined || files[1] !== undefined || files[2] !== undefined || files[3] !== undefined"
                  >上传图片</el-button>
                </el-col>
              </el-row>
            </div>

            <div class="edit-collection-upload">
              <el-upload
                drag
...
export default {
  ...
  async created() {
    this.colId = this.$route.params["collectionId"];
    ...
    if (this.colId !== undefined) {
      await this.$store.dispatch("Collections/getCollectionFromDB", this.colId);

      this.onUse = this.collections.onUse === 1;
      this.intro = this.collections.intro;
      this.productId = this.collections.productId;

      this.ruleForm.colTitle = this.collections.colTitle;
      ...
      this.initImages();
      if (this.collections.colType !== 0) {
        this.typeValue = this.colTypeState[this.collections.colType - 1].value;
      }
    }
  },
  ...
  methods: {
    ...
    initTableData() {
      ...
      for (const i in tmp) {
        ...
        item.id = this.items[tmp[i]].id;
        item.itemName = this.items[tmp[i]].itemName;
        tmpTable.push(item);
      }
    ...
    initImages() {
      this.imgsel = 1;
      //this.imagesUrl.push(this.collections.pic);
      this.imagesUrl[0] = this.collections.pic;

      /* for (let i = 1; i < this.collections.allPic.length; i++) {
    ...
    async submitCollection() {
      ...
      this.collections.pic = this.imagesUrl[0];
      this.collections.intro = this.intro;
      ...
      if (this.collections.colType === undefined) {
        this.collections.colType = 1;
      }
      ...
      for (let i = 0; i < this.imagesUrl.length; i++) {
        if (this.imagesUrl[i] === "") {
          ...
          console.log("sss", this.collections.allPic);
        }
      } */
      if (this.colId === undefined) {
        let collections = {
        ...
        collections.category = this.collections.category;
        collections.colTitle = this.collections.colTitle;
        collections.colType = this.collections.colType;
        collections.pic = this.collections.pic;
        collections.intro = this.collections.intro;
        collections.onUse = this.collections.onUse;
        collections.productId = this.collections.productId;
    ...
    async submitUpload() {
      ...
      for (let i = 0; i < this.files.length; i++) {
        let fileResponse = await this.httpHandler.uploadFile(
          `collection/${this.collections.colType}/`,
          this.files[i],
          this.showProgress
        );

        this.imagesUrl[i] = fileResponse;
      }
    },
...
.edit-collection-upload {
  margin-bottom: 20px;
  height: 400px;
}
.edit-collection-upload .el-upload {
  display: inline-block;
  text-align: center;
  cursor: pointer;
  outline: 0;
...
.edit-collection-upload .el-upload-dragger {
